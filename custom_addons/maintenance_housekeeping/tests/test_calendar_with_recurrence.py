...
class TestCalendarWithRecurrence(HttpCase):
    ...
    def test_dblclick_event_from_calendar(self):
        self.env['maintenance_housekeeping.team'].create({
            'name': 'the boys',
        })
        equipment = self.env['maintenance_housekeeping.equipment'].create({
            'name': 'room'
        })
        self.env['maintenance_housekeeping.request'].create({
            'name': 'send the mails',
            'schedule_date': datetime.today() - timedelta(weeks=2),
        })
        request = self.env['maintenance_housekeeping.request'].create({
            'name': 'clean the room',
