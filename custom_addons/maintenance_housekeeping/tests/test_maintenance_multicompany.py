...
class TestEquipmentMulticompany(TransactionCase):
    ...
    def test_00_equipment_multicompany_user(self):
        ...
        # Use full models
        Equipment = self.env['maintenance_housekeeping.equipment']
        MaintenanceRequestHouseKeeping = self.env['maintenance_housekeeping.request']
        Category = self.env['maintenance_housekeeping.equipment.category']
        ResUsers = self.env['res.users']
        ResCompany = self.env['res.company']
        MaintenanceTeamHouseKeeping = self.env['maintenance_housekeeping.team']
        ...
        team = MaintenanceTeamHouseKeeping.with_user(equipment_manager).create({
            'name': 'Metrology',
            'company_id': company_a.id,
        })
        # create a maintenance team for company B user
        teamb = MaintenanceTeamHouseKeeping.with_user(equipment_manager).with_context(allowed_company_ids=cids).create({
            'name': 'Subcontractor',
            'company_id': company_b.id,
        })
        ...
        with self.assertRaises(AccessError):
            MaintenanceTeamHouseKeeping.with_user(user).create({
                'name': 'Subcontractor',
                'company_id': company_b.id,
            })
        ...
        with self.assertRaises(AccessError):
            self.env['maintenance_housekeeping.stage'].with_user(user).create({
                'name': 'identify corrective maintenance requirements',
            })

        # Create an maintenance request for ( User Follower ).
        MaintenanceRequestHouseKeeping.with_user(user).create({
        ...
        MaintenanceRequestHouseKeeping.with_user(equipment_manager).create({
        ...
        self.assertEqual(MaintenanceRequestHouseKeeping.with_user(equipment_manager).with_context(allowed_company_ids=cids).search_count([]), 2)
        self.assertEqual(MaintenanceRequestHouseKeeping.with_user(user).search_count([]), 1)
