...
class MealCode(models.Model):
    ...
    arabic_abbreviation = fields.Char(
        string="Arabic Abbreviation", tracking=True)
    # posting_item = fields.Char(string="Posting Item",tracking=True)
    posting_item = fields.Many2one(
        'posting.item', string="Posting Item", ondelete="cascade", tracking=True)
    price_pax = fields.Integer(string="Price/Pax", tracking=True, default=1)
    price_child = fields.Integer(
        string="Price/Child", tracking=True, default=1)
    price_infant = fields.Integer(
        string="Price/Infant", tracking=True, default=1)

    @api.onchange('price_pax', 'price_child', 'price_infant')
    def _onchange_price_fields(self):
        for field_name in ['price_pax', 'price_child', 'price_infant']:
...
class MealPattern(models.Model):
    ...
    meal_pattern = fields.Char(
        string="Meal Pattern", required=True, tracking=True)
    description = fields.Char(string="Description",
                              required=True, tracking=True)
    abbreviation = fields.Char(string="Abbreviation", tracking=True)
    arabic_description = fields.Char(
    ...
    meal_posting_item = fields.Many2one(
        'posting.item', string="Meal Posting Item", ondelete="cascade", tracking=True)
    user_sort = fields.Integer(string="User Sort", tracking=True)
    meals_list_ids = fields.One2many(
        'meal.list', 'meal_pattern_id', string="Meals List")
...
class MealList(models.Model):
    ...
    meal_code = fields.Many2one(
        'meal.code', string="Meal Code", required=True, tracking=True)
    name = fields.Char(string="Name", tracking=True)
    quantity = fields.Integer(string="Quantity", default=1, tracking=True)
    apply_on_arrival = fields.Boolean(
        string="Apply on Arrival", default=False, tracking=True)
    apply_on_departure = fields.Boolean(
        string="Apply on Departure", default=False, tracking=True)
    apply_during_stay = fields.Boolean(
        string="Apply During Stay", default=False, tracking=True)
    meal_pattern_id = fields.Many2one(
        'meal.pattern', string="Meal Pattern", ondelete='cascade', tracking=True)
...
class AuthorizedCode(models.Model):
    ...
    can_authorize_complimentary = fields.Boolean(
        string="Complimentary", default=False, tracking=True)
    can_authorize_vip = fields.Boolean(
        string="VIP", default=False, tracking=True)
    can_authorize_house_use = fields.Boolean(
        string="House Use", default=False, tracking=True)
    can_authorize_out_of_order = fields.Boolean(
        string="Out of Order", default=False, tracking=True)
    can_authorize_room_on_hold = fields.Boolean(
        string="Room on Hold", default=False, tracking=True)
...
class RentableAsset(models.Model):
    ...
    create_as_with_guest = fields.Boolean(
        string="Create as (With Guest)", default=False, tracking=True)
    use_as_default_rent_notes = fields.Boolean(
        string="Use As Default Rent Notes", default=False, tracking=True)
    ...
    default_quantity = fields.Integer(
        string="Def. Quantity", default=0, tracking=True)
    ...
    extra_bed = fields.Boolean(
        string="Extra Bed", default=False, tracking=True)
    notes = fields.Text(string="Notes", tracking=True)
...
class TelephoneExtension(models.Model):
    ...
    extension_number = fields.Char(
        string="Telephone Extension", required=True, tracking=True)
    room_number = fields.Char(string="Room Number",
                              required=True, tracking=True)
