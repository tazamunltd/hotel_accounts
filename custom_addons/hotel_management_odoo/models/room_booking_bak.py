...
class RoomBooking(models.Model):
    ...
    state = fields.Selection(selection=[('draft', 'Draft'),
                                        ...
                                        ('check_out', 'Check Out'),
                                        ('cancel', 'Cancelled'),
                                        ('done', 'Done'),('no_show', 'No Show'),('block', 'Block'), ('confirmed', 'Confirmed'), ('not_confirmed', 'Not Confirmed'),('wait', 'Waiting list')], string='State',
    ...
    #
    # def _find_rooms_for_total_people(self):
    #     total_people_needed = self.total_people
    #
    #     # Get all rooms that can accommodate at least one person
    #     available_rooms = self.env['hotel.room'].search([('num_person', '>=', total_people_needed)])
    #
    #     # Sort rooms by num_person in ascending order to prioritize the closest match
    #     sorted_rooms = available_rooms.sorted(key=lambda r: r.num_person)
    #
    #     # Return the closest matching room or combination of rooms
    #     if sorted_rooms:
    #         # Return the first room with the closest matching capacity
    #         return sorted_rooms[:1]  # or use logic for multiple rooms if needed
    #     return []
    #
    # def button_find_rooms(self):
    #     # Find valid room(s) combinations
    #     rooms = self._find_rooms_for_total_people()
    #
    #     if rooms:
    #         # Clear existing room lines
    #         self.room_line_ids = [(5, 0, 0)]  # Removes all lines
    #
    #         # Ensure checkin_date and checkout_date are set
    ...
    #         # Add the best matching rooms to room_line_ids
    #
    #         for room in rooms:
    ...
    #             })]
    #     else:
    #         raise UserError('No suitable room combination found for the total number of people.')
    ...
    def _find_rooms_for_capacity(self):
        ...
        # Fetch available rooms that are not already used in this booking
        ...
        if not available_rooms:
            raise UserError('No more rooms available for booking.')

        if len(available_rooms) < rooms_to_add:
            raise UserError(f'Only {len(available_rooms)} rooms are available. You requested {rooms_to_add} rooms.')
    ...
    def increase_room_count(self):
        for record in self:
            ...
            total_available = self.env['hotel.room'].search_count([('status', '=', 'available')])
            currently_booked = len(record.room_line_ids)

            if (record.room_count + 1) <= (total_available + currently_booked):
                record.room_count += 1
                record.rooms_to_add = record.room_count - len(record.room_line_ids)
            else:
                raise UserError(f'Cannot increase rooms. Only {total_available} rooms available.')

    def decrease_room_count(self):
        for record in self:
            if record.room_count > len(record.room_line_ids):
                record.room_count -= 1
                record.rooms_to_add = record.room_count - len(record.room_line_ids)
            elif record.room_count > 1:
                raise UserError('Please remove rooms from the booking before decreasing room count.')
            else:
                raise UserError('Minimum one room is required for booking.')
