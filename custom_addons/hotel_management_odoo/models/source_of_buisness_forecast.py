...
latest_line_status AS (
    SELECT DISTINCT ON (rbl.id, ds.report_date)
           ...
           rb.source_of_business, -- Include source_of_business here
...
expected_arrivals AS (
    SELECT
        ...
        lls.company_id,
        lls.room_type_id,
        lls.source_of_business, -- Group by source_of_business
...
final_report AS (
    SELECT
        ...
        sb.source AS source_of_business, -- Renamed source_name to source_of_business
        ...
        (COALESCE(ih.in_house_count, 0) + COALESCE(ea.expected_arrivals_count, 0) - COALESCE(ed.expected_departures_count, 0)) AS expected_in_house,
        (COALESCE(ih.in_house_adults, 0) + COALESCE(ea.expected_arrivals_adults, 0) - COALESCE(ed.expected_departures_adults, 0)) AS expected_in_house_adults,
        (COALESCE(ih.in_house_children, 0) + COALESCE(ea.expected_arrivals_children, 0) - COALESCE(ed.expected_departures_children, 0)) AS expected_in_house_children,
        (COALESCE(ih.in_house_infants, 0) + COALESCE(ea.expected_arrivals_infants, 0) - COALESCE(ed.expected_departures_infants, 0)) AS expected_in_house_infants
    FROM date_series ds
    ...
    LEFT JOIN public.source_business sb ON sb.id = ea.source_of_business -- Join to get source_of_business
)
...
SELECT
    ...
    fr.company_id,
    fr.room_type_name,
    fr.source_of_business, -- Renamed column
...
GROUP BY fr.report_date, fr.company_id, fr.room_type_name, fr.source_of_business
ORDER BY fr.report_date, fr.company_id, fr.room_type_name, fr.source_of_business;
