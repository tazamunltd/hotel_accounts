    ...
    def action_run_process_room_availability(self):
        ...
        if self.env.context.get('filtered_date_range'):
            ...
            return {
                ...
                'views': [
                    (self.env.ref(
                        'hotel_management_odoo.view_hotel_room_result_tree').id, 'tree'),
                    (self.env.ref(
                        'hotel_management_odoo.view_hotel_room_result_pivot').id, 'pivot'),
                    (self.env.ref(
...
inventory AS (
    SELECT
        ...
        BOOL_OR(hi.overbooking_allowed) AS overbooking_allowed
    FROM hotel_inventory hi
    JOIN hotel_room hr 
      ON hr.company_id    = hi.company_id
     AND hr.room_type_name = hi.room_type
     AND hr.active = TRUE         -- only include active rooms here!
