...
class MonthlyAllotmentCharts(models.Model):
    ...
    def action_run_process_by_monthly_allotment(self):
        ...
        if self.env.context.get('filtered_date_range'):
            ...
            return {
                'name': _('Monthly Allotment Chart'),
                'type': 'ir.actions.act_window',
                'res_model': 'monthly.allotment.charts',
                'view_mode': 'tree,graph,pivot',
                'views': [
                    (self.env.ref(
                        'hotel_management_odoo.view_monthly_allotment_charts_tree').id, 'tree'),
                    (self.env.ref(
                        'hotel_management_odoo.view_monthly_allotment_charts_graph').id, 'graph'),
                    (self.env.ref(
