...
def draw_table_with_page_break(
    ...
    if row_height > usable_height:
        logger.debug("Warning:: row_height is larger than the usable page space.")
...
class WebAppController(http.Controller):
    ...
    def dump_data_to_redis(self, key, data, expiry=3600):
        ...
        logger.debug(f"Dumped all data on {key} to Redis.")
    ...
    def get_posting_items(self, hotel_id):
        ...
        # logger.debug("POSTING ITEM", posting_items)
        # logger.debug("MEAL DATA", meal_data)
        ...
        # logger.debug("POSTING ITEM IDS", posting_item_ids)
        ...
        # logger.debug the filtered posting items
        # logger.debug("FILTERED POSTING ITEMS", filtered_posting_items, len(filtered_posting_items))
    ...
    def forget_password(self, **kwargs):
        ...
        logger.debug("NEW RESET PASSWORD", new_password)
    ...
    def get_hotels(self, **kwargs):
        ...
        redis_client = self.get_redis_connections()
        # logger.debug("REDIS CLIENT", redis_client)
        ...
        if redis_client:
            try:
                ...
                for key in redis_keys:
                    ...
                    # logger.debug("CACHE DATA", cached_data)
                    if cached_data:
                        # logger.debug("Data found in Redis")
                        redis_data[key] = json.loads(cached_data)
                        # return json.loads(cached_data)

                # logger.debug("LENGTH REDIS", redis_data.keys())
        ...
        # logger.debug("INVENTORY", len(companies.inventory_ids))
        # room_types = self.get_model_data('room.type')
        # logger.debug("ROOM TYPES", room_types)
        ...
        for hotel in companies:
            if hotel.rate_code:
                ...
                # logger.debug("DEFAULT RATE CODE", rate_code.code)
            ...
            if len(rate_details) == 0:
                logger.debug("DATE RANGE NOT FOUND, TAKING THE DEFAULT RATE CODE")
                rate_details = request.env['rate.detail'].sudo().search([
                    ('rate_code_id', '=', rate_code.id)
                ])
            ...
            for room_type in room_types:
                # logger.debug("CHECK", room_type.description)
                ...
                # logger.debug("AV", len(available_rooms), room_type.description, hotel.name)
        ...
        #     logger.debug("POSTING ITEM", posting_item, not posting_item["show_in_website"])
    ...
    def get_hotel_by_id(self, hotel_id, **kwargs):
        ...
        for room_type in room_types:
            ...
            available_rooms = request.env['room.booking'].sudo().get_available_rooms_for_hotel_api(check_in,
                                                                                                   ...
                                                                                                   room_count, True,
                                                                                                   hotel.id)
            # logger.debug("AV", len(available_rooms), available_rooms)
            available_rooms_dict['id'] = room_type.id
            available_rooms_dict['type'] = room_type.description
            available_rooms_dict['pax'] = person_count
            available_rooms_dict['room_count'] = len(available_rooms)
    ...
    def check_multiple_room_availability(self, **kwargs):
        ...
        # logger.debug("PARTNER DOMAIN", partner_domain, len(partner_domain))
        ...
        for hotel in companies:
            ...
            # logger.debug("RATE DETAILS", rate_detail)
            # logger.debug("DETAILS", rate_detail.rate_detail_dicsount, rate_detail.is_amount, rate_detail.is_percentage)
            ...
            for room_type in room_types:
                ...
                # logger.debug("ROOM TYPE SPECIFIC RATE", room_type_specific_rates, room_type.id, hotel.id)
                ...
                for pax in pax_data:
                    ...
                    if len(room_type_specific_rates) > 0:
                        ...
                        for day in days:
                            # logger.debug("DAY PAX", day, pax['person_count'])
    ...
    def get_room_rates(self, **kwargs):
        ...
        # logger.debug(room_type_specific_rates)
        adult_price = 0
        child_price = 0
        ...
        for day in days:
            ...
            # logger.debug("room_type_specific_rates[_var]", room_type_specific_rates[_var])
    ...
    def confirm_room_availability(self, **kwargs):
        ...
        for room in rooms:
            ...
            # logger.debug("RATE CODE", rate_code)
            ...
            for adult in room.get('adult_details', []):
                ...
                # logger.debug('existing_adult', existing_adult)
            ...
            for item in services:
                ...
                if item.get('type', 0) == 1 or item.get('type', 0) == 3:
                    ...
                    # logger.debug("Posting Item", posting_item)

                    if posting_item.exists():
                        # logger.debug("list entry", posting_item.id)
                        ...
                        logger.debug(f"No posting item found for title: {item['title']}")
        ...
        #
        # # Return the result
        # logger.debug("BOOKING", new_booking)
        # return {
        #     "status": "success",
        #     "booking_id": new_booking.id,
    ...
    def retrieve_room_booking(self, **kwargs):
        ...
        # logger.debug("Partner", partner_id, partner_email)
        ...
        # logger.debug("LENGTH", len(bookings_data), len(group_bookings_data))
    ...
    def get_country_translation(self, country_name):
        ...
        #     "Spain": "إسبانيا",
        #     "Italy": "إيطاليا",
        #     'State of Palestine': 'الدولة الفلسطينية'
        #     # Add more as needed
        # }
        COUNTRY_TRANSLATIONS = {
    ...
    def _generate_pdf_for_booking(self, booking, forecast_lines=None): #Generate RC Individual
        ...
        if platform.system() == "Windows":
            ...
            user_profile = os.environ.get("USERPROFILE", "")  # On Windows, the user profile is typically in USERPROFILE
            if not user_profile:
                raise ValueError("Unable to determine the user profile path.")

            # Check if the font exists at the default path, and fall back to another path if needed
            font_path = os.path.join(user_profile, "Downloads", "dejavu-fonts-ttf-2.37", "dejavu-fonts-ttf-2.37", "ttf", "DejaVuSans.ttf")
            
            # Check if the font exists at the first path, and fall back to another location if needed
            if not os.path.exists(font_path):
                # Try another common location for Windows fonts (adjust the path if needed)
        ...
        #     font_path = "/usr/share/fonts/truetype/dejavu/DejaVuSans.ttf"  # Or "/usr/share/fonts/truetype/amiri/Amiri-Regular.ttf"
        #     arabic_val = 19
        #     logger.debug("ARABIC VALUES for the length values:{arabic_val}")
        #     # Register the font
        # try:
        #     pdfmetrics.registerFont(TTFont('CustomFont', font_path))
        #     font_name = 'CustomFont'
        ...
        if booking.use_price or booking.use_meal_price:
            ...
            manual_text = "يدوي" if user_lang.startswith("ar") else "Manual"
            
            display_rate_code = manual_text if booking.use_price else (booking.rate_code.description if booking.rate_code else "")
            display_meal_code = manual_text if booking.use_meal_price else (booking.meal_pattern.description if booking.meal_pattern else "")
        ...
        if booking.is_agent:
            ...
            # Get user language for localized text
            user_lang = request.env.user.lang or "en"
            individual_text = "فرد" if user_lang.startswith("ar") else "Individual"
            company_value = individual_text
        ...
        guest_details = [
            ...
            "No. of child:", f"{booking.child_count * booking.room_count}", "عدد الأطفال:                 "),

            ("Meal Pattern:", f"{self._truncate_text_(display_meal_code)}", "نظام الوجبات:            ", 
            "No. of infants:", f"{booking.infant_count * booking.room_count}", "عدد الرضع:                   "),
        ...
        payment_details = [
            ("Payment Method:", f"{self._truncate_text_(pay_type)}", "نظام الدفع:                ", 
             "Room Rate(Avg):", f"{avg_rate}", "متوسط سعر الغرف اليومي:"),
    ...
    def calculate_dynamic_meal_distribution(self, total_rate, meal_values, num_rooms, pax, discount, is_line_wise, nights, meal_is_percentage,bookings_count, rate_forecast_count):
        """
        ...
        logger.debug("Sum of unit prices (total_value): %s", total_value)
        ...
        for name, ua in meal_values.items():
            ...
            # Check if meal_is_percentage is True, then apply discount as percentage
            logger.debug(f"DEBUG: meal_is_percentage={meal_is_percentage}, discount={discount}, final_amt={final_amt}")
            if meal_is_percentage:
                # For percentage discount, apply percentage directly to final_amt
                # Note: discount is already in decimal form (0.05 = 5%), so don't divide by 100
                discount_amount = final_amt * discount
                final_amount_after_discount = final_amt - discount_amount
                logger.debug(f"DEBUG: Applying percentage discount - discount_amount={discount_amount}, final_amount_after_discount={final_amount_after_discount}")
            else:
                # For fixed amount discount, distribute the fixed amount across meals
                if is_line_wise:
                    # fixed_discount_per_meal = (num_rooms * discount * nights) * pct
                    fixed_discount_per_meal = (num_rooms * discount * rate_forecast_count) * pct
                else:
                    # fixed_discount_per_meal = (nights * (discount/bookings_count)) * pct
                    fixed_discount_per_meal = (rate_forecast_count * (discount/bookings_count)) * pct
                final_amount_after_discount = final_amt - fixed_discount_per_meal
                logger.debug(f"DEBUG: Applying fixed discount - fixed_discount_per_meal={fixed_discount_per_meal}, final_amount_after_discount={final_amount_after_discount}")
        ...
        logger.debug("Completed meal distribution: %s", distribution)
        return distribution
    ...
    def _generate_rc_pdf_for_booking(self, booking, forecast_lines=None, booking_ids=None, company_id=None): # GROUP BOOKING
        ...
        logger.debug("this is called for the test")
        buffer = BytesIO()
        c = canvas.Canvas(buffer, pagesize=A4)
        width, height = A4
        ...
        logger.debug(f"Final sorted room types: {room_types}")
        num_room_types = len(room_types)
        logger.debug(f"Number of room types: {num_room_types}")

        room_data = {
            rt: {'Rooms': 0, 'PAX': 0, 'Price': 0}
            for rt in room_types
        }
        logger.debug(f"Initialized room data: {room_data}")
        ...
        for gb in group_bookings:
            ...
            logger.debug(f"Processing booking {gb.id} with room type: {room_type}")
            # room_types_set.add(room_type)
            logger.debug(f"Room types set: {room_types_set}")
        ...
        # for gb in group_bookings:
            ...
            nights       = gb.no_of_nights or 1
            discount_per_night = gb.discount
            ...
            if rate_details and rate_details.price_type == 'room_only':
                ...
                if rate_details and rate_details.is_percentage:
                    rate_detail_discount_value = rate_details.rate_detail_dicsount or 0
                else:
                    rate_detail_discount_value = rate_details.rate_detail_dicsount if rate_details else 0

            logger.debug(f"Rate detail discount value: {rate_detail_discount_value}")

            logger.debug(f"Booking {gb.id} - State: {gb.state}, Room Type: {room_type}, Use Price: {gb.use_price}")
            logger.debug(f"Booking {gb.id} - total_rate_forecast: {gb.total_rate_forecast}, total_rate_after_discount: {gb.total_rate_after_discount}")
            
            if gb.use_price:
                if gb.state in ['not_confirmed', 'confirmed']:
                    raw_total = (gb.total_rate_forecast or 0)
                    logger.debug(f"Booking {gb.id} - Using forecast for confirmed/not_confirmed: {raw_total}")
                    room_count_ = gb.room_count or 0
                    room_discount = gb.room_discount or 0
                    if gb.room_is_percentage:
                        raw_total = raw_total - (room_discount * raw_total)
                    else:
                    ...
                    logger.debug(f"Booking {gb.id} - After room discount: {raw_total}")
                    ...
                    logger.debug(f"Booking {gb.id} - Using after_discount for block: {raw_total}")
                else:
                    raw_total = gb.total_rate_after_discount or 0
                    logger.debug(f"Booking {gb.id} - Using after_discount for other states: {raw_total}")
            else:
                ...
                logger.debug(f"Booking {gb.id} - Using after_discount for non-use_price booking: {raw_total}")

            logger.debug("Booking %s final raw_total: %s", gb.id, raw_total)
            ...
            if gb.use_meal_price:
                ...
                for booking_ in bookings:
                    ...
                    logger.debug(f"DEBUG BEFORE CALL: gb.meal_is_percentage={booking_.meal_is_percentage}, booking_.meal_discount={booking_.meal_discount}")
                    result = self.calculate_dynamic_meal_distribution(
                        ...
                        is_line_wise,
                        nights,
                        gb.meal_is_percentage,
                        bookings_count,
                        len(booking_.rate_forecast_ids)
                    ...
                    for meal_name, metrics in result.items():
                        ...
                        if gb.complementary_codes.type == "both" or gb.complementary_codes.type == "fb":
                            meal_data[meal_name]['Price'] += 0
                        else:
                ...
                if gb.meal_pattern:
                    ...
                    logger.debug(f"Booking {gb.id} has meal pattern {mp_rec.id} with {meal_count} meals")
                    logger.debug(f"Booking {gb.id} has meal pattern {mp_rec.id} with {len(mp_rec.meals_list_ids)} meals")
                    logger.debug(f"Booking {gb.id} has meal pattern {meals}")
                    meal_values = {}
                    for line in mp_rec.meals_list_ids:
                        if not (line.meal_code and line.meal_code.meal_code_sub_type):
                            continue
                        ...
                        if child_count > 0:
                            ...
                            meal_values[name] = ua
                    total_values_mp = sum(meal_values.values()) or 1.0
                    #if total then
                    if rate_details and rate_details.amount_selection == 'line_wise':
                        discount_share = (nights * rate_detail_discount_value * room_count)
                    else:
                        discount_share = (nights * rate_detail_discount_value)

                    if rate_details and rate_details.is_percentage:
                        logger.debug("Rate detail discount value is percentage")
                        logger.debug(f"Percentage discount rate: {rate_detail_discount_value}%")
                        # For percentage discount, we don't use discount_share as fixed amount
                        # Instead, we'll apply the percentage directly to each meal line

                    logger.debug(f"Rate detail discount value: {rate_detail_discount_value}, Discount share per meal: {discount_share}")
                    logger.debug(f"Using meal pattern {mp_rec.id}, lines={len(mp_rec.meals_list_ids)}, {mp_rec.meals_list_ids}")

                    # total_value_meals = sum(mp_rec.meals_list_ids.values()) or 1.0
                    # logger.debug((f"Using meal pattern {mp_rec.id}, lines={total_value_meals}"))
                    for line in mp_rec.meals_list_ids:
                        if not (line.meal_code and line.meal_code.meal_code_sub_type):
                            continue
                        ...
                        if child_count >0:
                            discount_pect = (ua + uc) / total_values_mp
                        else:
                            discount_pect = ua / total_values_mp
                        ...
                        if gb.state in ['not_confirmed', 'confirmed']:
                            ...
                            if rate_details and rate_details.price_type == 'room_only':
                                ...
                                # Apply discount based on whether it's percentage or fixed amount
                                if rate_details and rate_details.is_percentage:
                                    # Apply percentage discount directly to the line total
                                    percentage_discount = raw_line_total * (rate_detail_discount_value / 100)
                                    raw_line_total = raw_line_total - percentage_discount
                                    logger.debug(f"Applied {rate_detail_discount_value}% discount: {percentage_discount} on {raw_line_total + percentage_discount}")
                                else:
                                    # Apply fixed amount discount
                                    raw_line_total = raw_line_total - (discount_share * discount_pect)
                            meal_data[name]['PAX']   += adult_count * room_count
                            if gb.complementary_codes.type == "both" or gb.complementary_codes.type == "fb":
                                meal_data[name]['Price'] += 0
                            else:
                            ...
                            if rate_details and rate_details.price_type == 'room_only':
                                ...
                                if gb.complementary_codes.type == "both" or gb.complementary_codes.type == "room":
                                    meal_data[name]['Price'] += 0
                                else:
                                    # meal_data[name]['Price'] += raw_line_total
                                ...
                                if rate_details and rate_details.is_percentage:
                                    # Apply percentage discount directly to the line total
                                    percentage_discount = other_total * (rate_detail_discount_value / 100)
                                    other_total = other_total - percentage_discount
                                    logger.debug(f"Applied {rate_detail_discount_value}% discount: {percentage_discount} on {other_total + percentage_discount}")
                                else:
                                    # Apply fixed amount discount
                                    logger.debug(f"room_count ::{room_count}")
                                    if rate_details and rate_details.amount_selection == 'line_wise':
                                        other_total = other_total - (discount_share * discount_pect)
                                    else:
                                        logger.debug(f"room_count ::{discount_per_night} {nights} {discount_pect}")

                                        if nights != 0:
                                            if gb.state != 'block':
                                                other_total = other_total - (((discount_per_night/ 2) * nights) * discount_pect)
                                            else:
                                                group_id = gb.group_booking.id or gb.id
                                                bookings = request.env['room.booking'].search([
                                                    ('group_booking', '=', group_id),
                                                ...
                                                logger.debug(
                                                    f"block count ::{discount_per_night} {nights} {discount_pect}")
                                                if nights == 1:
                                                    if discount_per_night == 0:
                                                        discount_per_night = (discount_share/bookings_count) * 2 # added to match with meal and rate *2
                                                    logger.debug(f"other_total count before ::{other_total} {gb.id} {discount_per_night}")
                                                    other_total = other_total - ((discount_per_night/ 2) * discount_pect)
                                                    logger.debug(f"other_total after ::{other_total}")
                                                else:
                                                    other_total = other_total - (((discount_share) / bookings_count) * discount_pect)
                                                logger.debug(
                                                    f"other_total count ::{other_total} {nights} {discount_pect} {discount_share}")

                                            # other_total = other_total - ((discount_share / nights) * discount_pect)
                                        else:
                                            other_total = other_total - ((discount_share) * discount_pect)
                                meal_data[name]['PAX']   += adult_count
                                if gb.complementary_codes.type == "both" or gb.complementary_codes.type == "fb":
                                    ...
                                    # meal_data[name]['Price'] += raw_line_total
                                    meal_data[name]['Price'] += other_total
        ...
        if group_bookings:
            ...
            logger.debug("Booking ID: %s", booking.id)
            logger.debug("Group bookings data: %s", group_bookings_data)
            

            for booking_ in group_bookings_data:
                logger.debug("Booking: %s", booking_)
                if not booking_.parent_booking_name:
                    logger.debug("Booking if Not: %s", booking_)
                    total_nights += booking_.no_of_nights
                    logger.debug("Total nights: %s", total_nights)
                ...
                logger.debug("Room rate forecast: %s", room_rate_forecast)

                logger.debug(f"Booking {booking_.id} - State: {booking_.state}, Use Price: {booking_.use_price}")
                logger.debug(f"Booking {booking_.id} - total_rate_forecast: {booking_.total_rate_forecast}, total_rate_after_discount: {booking_.total_rate_after_discount}")
                ...
                if booking_.use_price:
                    if booking_.state in ['not_confirmed', 'confirmed']:
                        ...
                        base_amount = booking_.total_rate_forecast or 0
                        
                        if booking_.room_is_percentage:
                            # For percentage discount, apply percentage directly to base amount
                            # Note: room_discount is already in decimal form (0.05 = 5%), so don't divide by 100
                            room_discount_amount = base_amount * (booking_.room_discount or 0)
                            room_rate_value = base_amount - room_discount_amount
                            logger.debug(f"Booking {booking_.id} - Applying percentage room discount: {booking_.room_discount} on {base_amount} = {room_discount_amount}")
                        else:
                            # For fixed amount discount, distribute based on selection
                            if booking_.room_amount_selection == 'line_wise':
                                room_rate_value = base_amount - (
                            ...
                            logger.debug(f"Booking {booking_.id} - Applying fixed room discount: {booking_.room_discount}")

                        logger.debug(f"Booking {booking_.id} - Using forecast minus room discount: {room_rate_value}")
                    elif booking_.state == 'block':
                        # Use after_discount for block state
                        room_rate_value = booking_.total_rate_after_discount or 0
                        logger.debug(f"Booking {booking_.id} - Using after_discount for block: {room_rate_value}")
                    else:
                        room_rate_value = booking_.total_rate_after_discount or 0
                        logger.debug(f"Booking {booking_.id} - Using after_discount for other states: {room_rate_value}")
                else:
                    # For non-use_price bookings, always use total_rate_after_discount
                    # as it already contains the correct discount calculations
                    room_rate_value = booking_.total_rate_after_discount or 0
                    logger.debug(f"Booking {booking_.id} - Using after_discount for non-use_price booking: {room_rate_value}")
                
                total_room_rate += room_rate_value
                logger.debug("Total room rate: %s", total_room_rate)
                ...
                if booking_.state in ['not_confirmed', 'confirmed']:
                    ...
                    total_meal_rate  += booking_.total_meal_after_discount or 0
                logger.debug("Total meal rate: %s", total_meal_rate)
        ...
        guest_details = [
            ...
            "Company Name:",
            f"{self._truncate_text_(booking.company.name) if booking.is_grp_company else 'فرد' if user_lang.startswith('ar') else 'Individual' }" +
        ...
        # logger.debug(pay_type, 'line 4171',dict(booking.fields_get()['payment_type']['selection']))
        payment_details = [
        ...
        PAYMENT_ARABIC_ADJUSTMENTS = {
            ...
            "إجمالي سعر الغرف": -7,          # Total Room Rate
        ...
        # logger.debug('line 3991',bank_data , booking.contact_leader.id,)
        ...
        if bank_data:
            for detail in bank_data:
                ...
                # logger.debug(f"Account Number: {acc_number}, Bank Name: {bank_name}")
    ...
    def _generate_rc_guest_pdf_for_booking_main(self, booking, total_room_rate=None,
        ...
        user_lang = request.env.user.lang or "en"
        individual_text = "فرد" if user_lang.startswith("ar") else "Individual"
        booking_type = booking.partner_id.name  if booking.is_agent else individual_text
        ...
        for gb in group_bookings:
            ...
            logger.debug(f"room_type_name {room_type_name}")
        ...
        if booking.use_price or booking.use_meal_price:
            ...
            if booking.use_price:
                display_rate_code = manual_text
            else:
                display_rate_code = booking.rate_code.description if booking.rate_code else ""

            if booking.use_meal_price:
                display_meal_code = manual_text
            else:
                display_meal_code = booking.meal_pattern.description if booking.meal_pattern else ""
        ...
        guest_details = [
            ...
            ("Room Rate Min:", f"{min(rate_list) if rate_list else 0}", "سعر الغرف الأدنى:     ", "Meal Rate Min:", f"{min(meal_list) if meal_list else 0}",
             "سعر الوجبة الأدنى:        "),
            ("Room Rate Max:", f"{max(rate_list) if rate_list else 0}", "سعر الغرف الأعلى:    ", "Meal Rate Max:", f"{max(meal_list) if meal_list else 0}",
             "سعر الوجبة الأعلى:       "),
        ]
