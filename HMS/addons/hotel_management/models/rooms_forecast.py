...
class RoomsForecastReport(models.Model):
    ...
    def action_run_process_by_rooms_forecast(self):
        ...
        if self.env.context.get('filtered_date_range'):
            ...
            return {
                ...
                'views': [
                    (self.env.ref('hotel_management.view_rooms_forecast_tree').id, 'tree'),
                    (self.env.ref('hotel_management.view_rooms_forecast_pivot').id, 'pivot'),
                    (self.env.ref('hotel_management.view_rooms_forecast_graph').id, 'graph'),
                ],
                'target': 'current',
            }
        else:
    ...
    def run_process_by_rooms_forecast(self,from_date = None, to_date = None):
        ...
        _logger.debug("Started run_process_by_rooms_forecast")
...
ORDER BY fr.report_date, fr.company_id;
        ...
        _logger.debug(f"Query executed successfully for Room Forecast. {len(results)} results fetched")
