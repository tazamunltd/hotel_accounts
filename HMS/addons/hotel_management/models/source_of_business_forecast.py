...
class SourceOfBuisnessForecast(models.Model):
    _name = 'source.of.business.forecast'
    _description = 'Source of Business Forecast'
    ...
    @api.model
    def action_run_process_by_source_of_business_forecast(self):
        ...
        if self.env.context.get('filtered_date_range'):
            self.env["source.of.business.forecast"].run_process_by_source_of_business_forecast()

            return {
                'name': _('Source of Business Forecast Report'),
                'type': 'ir.actions.act_window',
                'res_model': 'source.of.business.forecast',
                'view_mode': 'tree,graph,pivot',
                'views': [
                    (self.env.ref(
                        'hotel_management.view_source_of_business_forecast_tree').id, 'tree'),
                    (self.env.ref(
                        'hotel_management.view_source_of_business_forecast_graph').id, 'graph'),
                    (self.env.ref(
                        'hotel_management.view_source_of_business_forecast_pivot').id, 'pivot'),
                ],
                'target': 'current',
            }
        else:
    ...
    def search_available_rooms(self, from_date, to_date):
        ...
        """
        self.run_process_by_source_of_business_forecast(from_date, to_date)
    ...
    def action_generate_results(self):
        ...
        try:
            self.run_process_by_source_of_business_forecast()
...
# ),
    ...
    def run_process_by_source_of_business_forecast(self,from_date = None, to_date = None):
        """Execute the SQL query and process the results."""
        _logger.debug("Started run_process_by_source_of_business_forecast")
