...
class DeletedReservationReport(models.Model):
    ...
    def action_run_process_by_deleted_reservation_status(self):
        ...
        if self.env.context.get('filtered_date_range'):
            ...
            return {
                ...
                'views': [
                    (self.env.ref('hotel_management.view_deleted_reservation_report_tree').id, 'tree'),
                    (self.env.ref('hotel_management.view_deleted_reservation_report_graph').id, 'graph'),
                    (self.env.ref('hotel_management.view_deleted_reservation_report_pivot').id, 'pivot'),
                ],
                'target': 'current',
            }
        else:
    ...
    def run_process_by_deleted_reservation_status(self, from_date = None, to_date = None):
        ...
        _logger.debug("Started run_process_by_deleted_reservation_status")
...
ORDER BY t.date_order;
        ...
        _logger.debug(f"Query executed successfully for Deleted. {len(results)} results fetched")
