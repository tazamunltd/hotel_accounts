...
class RoomSearch(models.Model):
    ...
    def search_available_rooms(self, from_date, to_date, room_count, company_id, room_type_id=None):
        ...
        try:
            ...
            # _logger.debug("Room search completed - Parameters: %s, Found: %d results", params, len(results))
            # _logger.debug("=== Detailed Room Search Results ===")
            ...
            #     )
            # _logger.debug("=== End of Room Search Results ===")
    ...
    def execute_room_search(self, query, params):
        ...
        try:
            ...
            _logger.debug("Executing room search with params: %s", formatted_params)
            
            self.env.cr.execute(query, params)
            results = self.env.cr.dictfetchall()
            
            # Log each result row with detailed information
            # _logger.debug("Room search results (%d rooms found):", len(results))
            # for idx, room in enumerate(results, 1):
            #     _logger.debug("Room %d: Type: %s, Company: %s, Free to Sell: %s, Total Rooms: %s, Overbooking: %s",
            #                 idx,
            #                 room.get('room_type_name', 'N/A'),
            #                 room.get('company_name', 'N/A'),
            #                 room.get('min_free_to_sell', 0),
    ...
    def _run_room_search_test(self):
        ...
        try:
            ...
            for test_case in test_cases:
                _logger.debug(f"Running test case: {test_case['name']}")
                ...
                _logger.debug(f"Test case '{test_case['name']}' passed")
            
            _logger.debug("All room search tests completed successfully")
            return True
