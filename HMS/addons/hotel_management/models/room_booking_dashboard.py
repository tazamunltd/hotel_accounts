...
class RoomBookingDashboard(models.Model):
    ...
    def get_dashboard_data(self, filters=None):
        ...
        """
        try:
            _logger.debug('Starting dashboard data fetch with filters: %s', filters)
            
            # Get company and system date
            company = self.env.company
            _logger.debug('Company ID: %s', company.id)
            ...
            _logger.debug(f'Min Max date: {min_date}, {max_date}')
            ...
            try:
                ...
                _logger.debug('Parsed dates - Start: %s, End: %s', start_date, end_date)
            ...
            if not start_date or not end_date:
                ...
                start_date = min_date
                end_date = max_date
                _logger.debug('Using default dates - Start: %s, End: %s', start_date, end_date)
            ...
            _logger.debug('Found %d booking records', len(bookings))
            ...
            # _logger.debug('Returning dashboard data: %s', response_data)
