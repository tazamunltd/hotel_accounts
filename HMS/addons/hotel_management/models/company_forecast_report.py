...
class CompanyForecastReport(models.Model):
    ...
    def action_run_process_by_company_forecast_report(self):
        ...
        if self.env.context.get('filtered_date_range'):
            ...
            return {
                ...
                'views': [
                    (self.env.ref(
                        'hotel_management.view_company_forecast_report_tree').id, 'tree'),
                    (self.env.ref(
                        'hotel_management.view_company_forecast_report_graph').id, 'graph'),
                    (self.env.ref(
                        'hotel_management.view_company_forecast_report_pivot').id, 'pivot'),
                ],
                'target': 'current',
            }
        else:
    ...
    def run_process_by_company_forecast_report(self,from_date = None, to_date = None):
        ...
        _logger.debug("Started run_process_by_company_forecast_report")
...
ORDER BY fr.report_date, fr.company_id, fr.room_type_name, fr.company_customer_id, fr.company_customer_name;   
     """
        ...
        _logger.debug(f"Query executed successfully Company. {len(results)} results fetched")
