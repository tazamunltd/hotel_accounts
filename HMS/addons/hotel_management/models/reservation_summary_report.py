...
class ReservationSummaryReport(models.Model):
    ...
    def action_run_process_by_reservation_summary_report(self):
        ...
        if self.env.context.get('filtered_date_range'):
            ...
            return {
                ...
                'views': [
                    (self.env.ref(
                        'hotel_management.view_reservation_summary_report_tree').id, 'tree'),
                    (self.env.ref(
                        'hotel_management.view_reservation_summary_report_graph').id, 'graph'),
                    (self.env.ref(
                        'hotel_management.view_reservation_summary_report_pivot').id, 'pivot'),
                ],
                'target': 'current',
            }
        else:
            ...
            domain = [
                ('first_arrival', '>=', default_from_date),
                ('last_departure', '<=', default_to_date),
            ]
            return {
                'name': _('Reservation Summary Report'),
                'type': 'ir.actions.act_window',
    ...
    def run_process_by_reservation_summary_report(self, from_date=None, to_date=None):
        ...
        _logger.debug("Started run_process_by_reservation_summary_report")
