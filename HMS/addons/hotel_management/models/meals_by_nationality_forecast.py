    ...
    def action_run_process_by_meals_by_nationality_forecast(self):
        ...
        if self.env.context.get('filtered_date_range'):
            ...
            return {
                ...
                'views': [
                    (self.env.ref(
                        'hotel_management.view_meals_by_nationality_forecast_tree').id, 'tree'),
                    (self.env.ref(
                        'hotel_management.view_meals_by_nationality_forecast_graph').id, 'graph'),
                    (self.env.ref(
                        'hotel_management.view_meals_by_nationality_forecast_pivot').id, 'pivot'),
                ],
                'target': 'current',
            }
        else:
...
# ),
    ...
    def run_process_by_meals_by_nationality_forecast(self, from_date=None, to_date=None):
        ...
        _logger.debug("Started run_process_by_meals_by_nationality_forecast")
        ...
        _logger.debug("Existing records for the current company deleted")
