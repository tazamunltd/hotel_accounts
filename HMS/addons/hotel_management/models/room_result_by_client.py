...
class RoomResultByClient(models.Model):
    ...
    def action_run_process(self):
        ...
        if self.env.context.get('filtered_year_range'):
            ...
            return {
                ...
                'views': [
                    (self.env.ref('hotel_management.view_room_result_by_client_tree').id, 'tree'),
                    (self.env.ref('hotel_management.view_room_result_by_client_pivot').id, 'pivot'),
                    (self.env.ref('hotel_management.view_room_result_by_client_graph').id, 'graph'),
                ],
                'target': 'current',
            }
        else:
    ...
    def run_process(self,from_date = None, to_date = None):
        ...
        _logger.debug("Started run_process")
        # SQL Query (modified)
