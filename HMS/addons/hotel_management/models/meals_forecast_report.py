...
class MealsForecastReport(models.Model):
    ...
    def action_run_process_by_meals_forecast(self):
        ...
        if self.env.context.get('filtered_date_range'):
            ...
            return {
                ...
                'views': [
                    (self.env.ref(
                        'hotel_management.view_meals_forecast_report_tree').id, 'tree'),
                    (self.env.ref(
                        'hotel_management.view_meals_forecast_report_pivot').id, 'pivot'),
                    (self.env.ref(
                        'hotel_management.view_meals_forecast_report_graph').id, 'graph'),
                ],
                'target': 'current',
            }
        else:
    ...
    def run_process_by_meals_forecast(self, from_date=None, to_date=None):
        ...
        _logger.debug("Started run_process_by_meals_forecast")
...
meal_pattern_names AS (
    SELECT 
        ...
        COALESCE(jsonb_extract_path_text(mcst.name::jsonb, 'en_US'), '') AS meal_code_subtype_name
