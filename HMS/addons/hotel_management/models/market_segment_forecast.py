...
class MarketSegmentForecastReport(models.Model):
    ...
    def action_run_process_by_market_segment_forecast(self):
        ...
        if self.env.context.get('filtered_date_range'):
            ...
            return {
                ...
                'views': [
                    (self.env.ref(
                        'hotel_management.view_market_segment_forecast_tree').id, 'tree'),
                    (self.env.ref(
                        'hotel_management.view_market_segment_forecast_report_graph').id, 'graph'),
                    (self.env.ref(
                        'hotel_management.view_market_segment_forecast_report_pivot').id, 'pivot'),
                ],
                'target': 'current',
            }
        else:
    ...
    def run_process_by_market_segment_forecast(self,from_date = None, to_date = None):
        ...
        _logger.debug("Started run_process_by_market_segment_forecast")
