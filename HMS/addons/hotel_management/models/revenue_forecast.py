...
class RevenueForecast(models.Model):
    ...
    def action_run_process_by_revenue_forecast(self):
        ...
        if self.env.context.get('filtered_date_range'):
            ...
            return {
                ...
                'views': [
                    (self.env.ref(
                        'hotel_management.view_revenue_forecast_tree').id, 'tree'),
                    (self.env.ref(
                        'hotel_management.view_revenue_forecast_pivot').id, 'pivot'),
                    (self.env.ref(
                        'hotel_management.view_revenue_forecast_graph').id, 'graph'),
                ],
                'target': 'current',
            }
        else:
    ...
    def run_process_by_revenue_forecast(self,from_date = None, to_date = None):
        ...
        _logger.debug("Started run_process_by_revenue_forecast")
