...
class ApplicationRoomType(models.Model):
    ...
    name = fields.Char(string="Application Room Type", required=True, tracking=True,translate=True)
...
class MealPatternSubType(models.Model):
    ...
    def action_unarchieve_record(self):
        for record in self:
            record.active = True
    name = fields.Char(string="Main Meal Pattern", required=True, tracking=True,translate=True)
    company_id = fields.Many2one('res.company', string="Company", default=lambda self: self.env.company, tracking=True)
...
class MealPattern(models.Model):
    ...
    @api.constrains('meals_list_ids')
    def _check_meals_list_ids(self):
        for record in self:
            if not record.meals_list_ids:
                raise ValidationError(_("You must add at least one meal in Meals List."))
    # domain=lambda self: [
    #     ('company_id', '=', self.env.company.id)],)
