...
class DepartmentCode(models.Model):
    ...
    @api.depends('charge_group', 'payment_group', 'tax_group', 'adjustment')
    def _compute_charge_type(self):
        """Compute charge_type based on which group field is selected"""
        for record in self:
            if record.charge_group:
                record.charge_type = record.charge_group
            elif record.payment_group:
                record.charge_type = record.payment_group
            elif record.tax_group:
                record.charge_type = record.tax_group
            elif record.adjustment:
                record.charge_type = record.adjustment
            else:
                record.charge_type = False
...
class PostingItem(models.Model):
    ...
    main_department = fields.Many2one('department.code', string="Main Department",tracking=True,required=True)
