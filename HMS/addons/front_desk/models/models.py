...
class OutOfOrderManagement(models.Model):
    _name = 'out.of.order.management.front.desk'
    ...
    # Codes and Comments
    out_of_order_code = fields.Char(string=_("Out of Order Code"), tracking=True, translate=True) 
    authorization_code = fields.Char(string=_("Authorization Code"), tracking=True, translate=True)
...
class RoomsOnHoldManagement(models.Model):
    ...
    # Codes and Comments
    roh_code = fields.Char(string=_("ROH Code"), tracking=True, translate=True)
    authorization_code = fields.Char(string=_("Authorization Code"), tracking=True, translate=True)
    comments = fields.Text(string="Comments", tracking=True)
...
# -----------------------Front Desk Room Booking------------------------------
class RoomBookingFrontDesk(models.Model):
    """Model that handles the hotel room booking and all operations related
     to booking"""
    _name = "room.booking.front.desk"
    ...
    def check_waiting_list_and_notify(self):
        ...
        if not canceled_bookings:
            _logger.debug("No canceled bookings found for today.")
            return
    ...
    def _compute_is_checkin_in_past(self):
        ...
        if system_date:
            ...
            print(f"LIne 506 System Date: {system_date}")
        else:
        ...
        for record in self:
            if record.checkin_date:
                ...
                record.is_checkin_in_past = checkin_date < system_date
            else:
                record.is_checkin_in_past = False
    ...
    def move_to_no_show(self):
        ...
        yesterday = datetime.combine((system_date - timedelta(days=1)).date(), time.max)
        ...
        for booking in bookings:
            try:
                ...
                _logger.debug(f"Failed to process booking ID {booking.id}: {e}")
        ...
        for booking in bookings:
            ...
            _logger.debug(f"Updated booking ID {booking.id} to 'no_show'")
    ...
    def retrieve_dashboard(self):
        ...
        print('863', system_date, end_of_day)
        print('864',system_date + timedelta(days=1))
        ...
        print('913', today_checkout_count)
    ...
    def _compute_total_total_forecast(self):
        for record in self:
            ...
            _logger.debug(f"Total days {total_days}")
            if total_days == 0:
                total_days = 1
    ...
    def get_all_prices(self, forecast_date, record):
        ...
        if not record.use_meal_price:
            ...
            if rate_details.price_type == "rate_meals":
                _logger.debug("RATE AND MEALS pricing detected, setting take_meal_price to True")
                take_meal_price = True

            if rate_details.price_type == "room_only":
                _logger.debug("ROOM ONLY pricing detected, setting take_meal_price to False")
                take_meal_price = False

            # Check if meal_pattern.id exists
            if record.meal_pattern.id:
                _logger.debug("Meal pattern exists, setting take_meal_price to True")
                take_meal_price = True

            # elif rate_details.price_type == "rate_meals":
            #     _logger.debug("Meal pattern does not exist, setting take_meal_price to False")
            #     take_meal_price = False

        _logger.debug(f"Taking Meal Price {take_meal_price}")
        if take_meal_price:
            if not record.use_meal_price:
        ...
        _logger.debug(f"MEAL PRICE {meal_price}")
            ...
            if record.rate_code.id and not record.use_price:
                ...
                if rate_details:
                    ...
                    if record.hotel_room_type:
                        ...
                        print("ROOM TYPE SPECIFIC RATE", room_type_specific_rate)
                        _logger.debug(f"ROOM TYPE SPECIFIC RATE, {room_type_specific_rate} {room_type_specific_rate.id}")
    ...
    def _get_forecast_rates(self, from_rate_code=False):
        _logger.debug("GET FORECAST RATES")
        ...
        for record in self:
            ...
            _logger.debug(f"System Date {_system_date}")

            for rate_forecast in record.rate_forecast_ids:
                if rate_forecast:
                    # Ensure rate_forecast's room_booking_id is valid
            ...
            if record.state == 'check_in':
                # total_days = (checkout_date - _system_date).days
                # print("STATE IS CHECK IN", _system_date, total_days)
                from_start = False
            ...
            for day in range(total_days):
                ...
                _logger.debug(f"Changing Rates {forecast_date}, {record.state}, {_system_date}, {today_date}, {change}")
                if change:
                    ...
                    _logger.debug(
                        f"GETTING DISCOUNT, {before_discount}, {rate_detail_discount_value}, {is_percentage_value}, {is_amount_value}, {amount_selection_value}")
                    ...
                    if existing_forecast:
                        _logger.debug("Updating existing forecast")
                        ...
                        _logger.debug("Creating new forecast")
            ...
            # _logger.debug(f"FORECAST LINES {forecast_lines}")
    ...
    def write(self, vals):
        ...
        if 'state' in vals and 'parent_booking_id' in vals and 'parent_booking_name' in vals and room_line_count > 1:
            ...
            if room_line_count > 1:
                ...
                _logger.debug(final_room_line, "this is final",
    ...
    def _onchange_company_id(self):
        ...
        if self.company_id:
            self.checkin_date = self.company_id.system_date
    ...
    def action_confirm(self):
        # today = datetime.now().date()
        today = self.env.company.system_date.date()
        print("Today's System Date:", today)
        for record in self:
    ...
    def action_checkout(self):
        ...
        _logger.debug(f"The Posting Item Meal is: {posting_item_meal}")
        ...
        _logger.debug(f"Line Posting Items: {line_posting_items}")
        # if not self.posting_item_ids:
        #     raise ValidationError("You cannot proceed with checkout. Please add at least one Posting Item.")
        ...
        _logger.debug(f"Debug: Today's date = {today}")
        _logger.debug(f"Debug: checkout_date = {self.checkout_date}")
        ...
        booking_list = self._compute_amount_untaxed(True)
        _logger.debug(f"BOOKING LIST {booking_list}")
        if booking_list:
            account_move = self.env["account.move"].create([{
                'move_type': 'out_invoice',
                'invoice_date': today,
    ...
    def check_room_meal_validation(self, vals, is_write):
        _logger.debug(f"VALIDATION STARTED: {vals}, IS_WRITE: {is_write}")
        ...
        for record in self:
            if vals.get('use_price', record.use_price):
                ...
                # _logger.debug(f"TEST {vals.get('room_discount', record.room_discount) * 100}")
        ...
        _logger.debug("VALIDATION COMPLETED")
    ...
    #
    #             _logger.debug(f"MEAL DISCOUNT {record.meal_discount}")
